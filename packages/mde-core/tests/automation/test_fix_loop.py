...
def test_fix_loop_repairs_a_test_failure(tmp_path: Path) -> None:
    ...
    registry.register("ai.fix", lambda c, s: calls.append(s.command) or {"manifest_path": "fix"})
    ...
    result = execute_workflow_with_fix_loop(
        workflow(), WorkflowContext("feature", repository_root=tmp_path), registry, max_fix_attempts=2
    )
    assert result.status == "completed"
    assert calls == ["ai.generate", "change.apply", "test.run", "ai.fix", "change.apply", "test.run"]
...
def test_fix_loop_stops_after_limit(tmp_path: Path) -> None:
    ...
    registry.register("test.run", lambda c, s: (_ for _ in ()).throw(RuntimeError("still failing")))
