...
def test_normalize_usage_handles_provider_shapes():
    assert normalize_usage({"promptTokenCount": 10, "candidatesTokenCount": 5})["total_tokens"] == 15
...
def test_estimate_cost_uses_environment(monkeypatch):
    ...
    assert estimate_cost_usd("openai", "test-model", {"input_tokens": 1_000_000, "output_tokens": 500_000}) == 2.0
