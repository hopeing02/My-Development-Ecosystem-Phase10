...
class ClaudeProvider(BaseHTTPAIProvider):
    ...
    def build_request(self, request: AIRequest, *, api_key: str, model: str):
        return (
            ...
            {"model": model, "max_tokens": 8192, "system": SYSTEM_PROMPT, "messages": [{"role": "user", "content": self.user_prompt(request)}]},
        )

    def extract_text_and_usage(self, data: dict[str, Any]):
        chunks = [item.get("text", "") for item in data.get("content", []) if isinstance(item, dict) and item.get("type") == "text"]
